...
PASTA_ARMAZENA_ARQUIVO = './uploads'
PASTA_LOGS = './logs'
EXTENSOES_PERMITIDAS = {'pdf'}
TOKEN_SERVICO = 'SIMULANDO_TOKEN'
...
log_handler = RotatingFileHandler(os.path.join(PASTA_LOGS, 'app.log'), maxBytes=10000, backupCount=1)
...
def alterar_resolucao_pdf(caminho_pdf, resolucao):
    try:
        ...
        caminho_saida = caminho_pdf.replace('.pdf', f'_resolucao_{resolucao}.pdf')
...
def rota_alterar_resolucao_pdf():
    ...
    if arquivo and arquivo_permitido(arquivo.filename):
        ...
        if caminho_saida.startswith("Erro"):
            registra_log(f'Erro na redução de resolução do arquivo {nome_arquivo}: {caminho_saida}')
            return jsonify({'erro': caminho_saida}), 500

        nome_saida = os.path.basename(caminho_saida)
        registra_log(f'Redução de resolução concluída para o arquivo {nome_arquivo}, salvo como {nome_saida}')
